...
describe('DepenseService', () => {
  ...
  it('listDepenses retourne la liste fournie par le repo', async () => {
    const sample: Depense[] = [{ depenseId: '1', description: 'course de la semaine', montant: 41, userId: '1', enseigne: 'Intermarché', date: new Date("2026-01-10") } as unknown as Depense];
  ...
  it('getDepense retourne la dépense quand elle existe', async () => {
    const depense: Depense = { depenseId: '1', description: 'course de la semaine', montant: 41, userId: '1', enseigne: 'Intermarché', date: new Date("2026-01-10") } as unknown as Depense;
  ...
  it('createDepense appelle save et retourne la dépense créée', async () => {
    const input: Omit<Depense, 'depenseId'> = { description: 'course de la semaine', montant: 41, userId: '1', enseigne: 'Intermarché', date: new Date("2026-01-10") } as unknown as Omit<Depense, 'depenseId'>;
  ...
  it('updateDepense appelle modify et retourne la dépense modifiée', async () => {
    const input: Depense = { depenseId: '2', description: 'course de la semaine', montant: 42, userId: '1', enseigne: 'Intermarché', date: new Date("2026-01-10") } as unknown as Depense;
    const modified: Depense = { ...input } as unknown as Depense;
    mockRepo.modify.mockResolvedValue(modified);
    await expect(service.updateDepense(input)).resolves.toEqual(modified);
